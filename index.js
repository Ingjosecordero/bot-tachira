...
bot.on('text', async (ctx) => {
    ...
    if (state.step === 'esperando_articulo') {
        ...
        ctx.reply(`✅ Agregado: ${state.tempArt} (${cant})\n¿Desea agregar otro artículo?`, 
            Markup.inlineKeyboard([[Markup.button.callback('➕ Agregar Otro', 'ADD_MORE'), Markup.button.callback('💾 Finalizar', 'FINISH')]]));
    }
});
...
bot.on('callback_query', async (ctx) => {
    ...
    if (ctx.callbackQuery.data === 'ADD_MORE') {
        ...
        const res = await callApi({ op: 'registrar_salida', id: userId, art: state.items.join(','), zona: zona });
        delete userState[userId];
        ctx.reply(res && res.ok ? `✅ REPORTE GUARDADO EN ${zona}.` : `❌ Error: ${res.msg || "Fallo"}`);
    }
});
